...
export async function insertCallout(app, run_type = 2) {
  ...
  if (!/^[> ]*$/.test(line_prefix)) {
    ...
    let message = "The cursor (caret) is at a position where the callout cannot be inserted.";
    new Notice(message);
    console.log(message);
    return;
  }
  ...
  // Run Type: 1 ##################################################
    ...
    foldState = await runJS.suggest("Folding state of callout?", list, "Folding state");
    ...
    calloutContent = await runJS.prompt("Optional Content Text (Shift Enter to Insert New Line)", "", "Content", true)
  ...
  // Run Type: 2 ##################################################
    ...
    class CalloutModal extends Modal {
      ...
      onOpen() {
        ...
        // Enter title here
        // 제목 입럭
        this.title = callout_title.createDiv({ cls: "callout-title-inner", attr: { contenteditable: "plaintext-only", placeholder: "Enter title here" } });
        ...
        // Enter content here
        // 내용 입럭
        this.callout_editor = callout_content.createEl("p", { attr: { contenteditable: "plaintext-only", placeholder: "Enter content here" } });
