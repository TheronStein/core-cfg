...
M.reload_config = function()
  ...
  for module_name, _ in pairs(package.loaded) do
    ...
    if not module_name:match("^lazy") and not module_name:match("^fzf%-lua") and not module_name:match("^plenary") then
      package.loaded[module_name] = nil
      cleared = cleared + 1
    end
  end
  ...
  vim.notify(string.format("✓ Config reloaded (%d modules cleared)", cleared), vim.log.levels.INFO)
end
