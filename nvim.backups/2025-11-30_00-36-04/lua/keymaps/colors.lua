...
M.setup = function()
	...
	map("n", "<localleader>cc", function()
		...
		if word ~= "" then
			...
			vim.utils.notify("No color under cursor", vim.log.levels.WARN)
		end
	...
	-- Color statistics
	local function fetch_color_statistics()
		local colors = {}
	...
	if wk_ok then
		wk.register({
			["<localleader>cv"] = {
				{
					{ "<localleader>cv", group = "Visual tools" },
					{ "<localleader>cvc", fetch_color_statistics(), desc = "Count colors in buffer" },
					{ "<localleader>cvh", desc = "Highlight all colors" },
					{ "<localleader>cvs", desc = "Select all colors" },
				},
			},
		}, { mode = "n" })
	end
