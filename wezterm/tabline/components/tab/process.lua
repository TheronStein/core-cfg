...
local custom_icons = require("modules.icons.custom_icons")
...
return {
  ...
  update = function(tab, opts)
    ...
    if tab.active_pane and tab.active_pane.foreground_process_name then
      ...
      foreground_process_name = foreground_process_name:match("([^/\\]+)[/\\]?$")
    ...
    if foreground_process_name == "" or foreground_process_name == "wslhost.exe" then
      foreground_process_name = (tab.tab_title and #tab.tab_title > 0) and tab.tab_title
    ...
    if foreground_process_name == "wezterm" then
      foreground_process_name = tab.active_pane.domain_name ~= "local"
