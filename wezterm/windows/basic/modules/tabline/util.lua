...
local function require_component(object, v)
  ...
  if object.tab_id then
    component = 'gui.tabline.components.tab.' .. v
  else
    component = 'gui.tabline.components.window.' .. v
  end
  return component
end
  ...
  for _, v in ipairs(components_opts) do
    if type(v) == 'string' then
      if v == 'ResetAttributes' then
        ...
        local component_name = require_component(object, v)
        local debug_config = require('config.debug')
        if debug_config.is_enabled('debug_tabline_components') then
          wezterm.log_info('[TABLINE:COMPONENTS] Attempting to load:', component_name)
        end
        local ok, result = pcall(require, component_name)
        if ok then
          if debug_config.is_enabled('debug_tabline_components') then
            wezterm.log_info('[TABLINE:COMPONENTS] Successfully loaded:', component_name)
          end
          ...
          local debug_config = require('config.debug')
          if debug_config.is_enabled('debug_tabline_components') then
            wezterm.log_error('[TABLINE:COMPONENTS] Failed to load', component_name, ':', tostring(result))
          end
          table.insert(components, { Text = v .. '' })
        end
      end
    elseif type(v) == 'table' and type(v[1]) == 'string' then
      local component_name = require_component(object, v[1])
      if DEBUG then
        wezterm.log_info('Attempting to load component (table): ' .. component_name)
      end
      local ok, result = pcall(require, component_name)
      if ok then
        if DEBUG then
          wezterm.log_info('Successfully loaded component (table): ' .. component_name)
        end
        local opts = M.deep_copy(component_opts)
        if result.default_opts then
          opts = M.deep_extend(opts, result.default_opts)
  ...
  if opts.icons_enabled and opts.icon then
    ...
    if type(opts.icon) == 'table' then
      if opts.icon.align == 'right' then
        -- Apply custom color to text if specified
        if opts.color then
          if opts.color.bg then
            table.insert(icon_name, { Background = { Color = opts.color.bg } })
          end
          if opts.color.fg then
            table.insert(icon_name, { Foreground = { Color = opts.color.fg } })
          end
        end
        ...
        table.insert(icon_name, { Text = opts.icon[1] .. ' ' })
        -- Keep the same background for the text
        if opts.color then
          if opts.color.fg then
            table.insert(icon_name, { Foreground = { Color = opts.color.fg } })
          end
          -- Don't reset background - keep icon background
        else
          M.insert_elements(icon_name, reset_attributes)
          M.insert_elements(icon_name, attributes)
        end
