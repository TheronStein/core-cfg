...
WEZTERM_CONFIG="$HOME/.core/.sys/configs/wezterm"
...
RELOAD_SIGNAL="$RUNTIME_DIR/wezterm_theme_reload_${WORKSPACE_NAME}.txt"
...
cleanup() {
    ...
    sleep 0.2
    rm -f "$PREVIEW_FILE" "$OPACITY_FILE" "$RELOAD_SIGNAL"
}
...
# Generate fzf color scheme from theme colors
generate_fzf_colors() {
    local theme_name="$1"
...
export -f apply_preview generate_preview apply_to_workspace adjust_opacity generate_fzf_colors
export PREVIEW_FILE THEMES_FILE SCRIPT_DIR WORKSPACE_NAME PREVIEW_MODE OPACITY_FILE RELOAD_SIGNAL
export RED GREEN YELLOW BLUE CYAN MAGENTA BOLD DIM RESET

# Main browser
main() {
    # Ensure cleanup trap is set
    trap cleanup EXIT INT TERM

    # Try to get current theme for initial colors
    local current_theme=""
    if [[ -f "$PREVIEW_FILE" ]]; then
        local file_content=$(cat "$PREVIEW_FILE")
        if [[ "$file_content" != "INIT" ]] && [[ "$file_content" != "CANCEL" ]]; then
            current_theme="$file_content"
        fi
    fi

    # Generate initial fzf colors from current theme
    local fzf_colors=$(generate_fzf_colors "$current_theme")
    ...
    echo -e "${DIM}  Colors: Ctrl+U (update fzf colors to match current theme)${RESET}"
    echo -e "${DIM}  Opacity: [ (decrease) | ] (increase)${RESET}"
    echo

    # Run fzf with dynamic colors
    # Ctrl+U will exit and signal restart with updated colors
    local selected
    local fzf_result
    selected=$(
        generate_list | fzf \
            ...
            --bind="ctrl-u:execute-silent(echo RELOAD > '$RELOAD_SIGNAL' && sleep 0.15)+abort" \
    ...
    # Check if Ctrl+U was pressed to reload with new colors
    if [[ -f "$RELOAD_SIGNAL" ]]; then
        rm -f "$RELOAD_SIGNAL"
        # Get colors from currently previewed theme and restart
        local new_colors=$("$SCRIPT_DIR/get-current-fzf-colors.sh")
        if [[ -n "$new_colors" ]]; then
            echo -e "${CYAN}↻ Updating fzf colors to match current theme...${RESET}"
            sleep 0.2
            clear
            # Prevent cleanup trap from cancelling the theme
            trap - EXIT INT TERM
            # Don't write CANCEL to preview file
            main "$@"  # Restart with updated colors
            return
        fi
    fi
