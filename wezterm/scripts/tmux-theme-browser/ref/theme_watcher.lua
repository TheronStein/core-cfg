...
-- Get preview file path for current session
function M.get_preview_file()
	...
	if session then
		return string.format("/tmp/wezterm_preview_%s.txt", session)
	else
		return "/tmp/wezterm_preview_theme.txt"
	end
end

-- Start watching for theme changes
function M.start_watcher(window)
	if not window then
		return
	end
