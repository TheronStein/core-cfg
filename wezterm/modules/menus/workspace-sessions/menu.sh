#!/usr/bin/env bash
# WezTerm Workspace Sessions Menu
...
declare -a MENU_ITEMS=(
    ...
    "load|📂 Load Session"
    "create|🆕 Create New Session (Name → Icon → Color)"
    "choose_icon|🎨 Choose Template Icon"
    "choose_color|🌈 Choose Template Color"
    "separator1|─────────────────────────────"
    "list|📋 Session List View (Load/Delete/Rename)"
)
...
show_menu() {
    printf "%s\n" "${MENU_ITEMS[@]}" \
        | fzf \
            ...
            --layout=reverse \
            --border=rounded \
            --border-label="╣ Session Manager > Workspace Management ╠" \
            --prompt="Select ❯ " \
            --pointer="▶" \
            --marker="✓" \
            --delimiter='|' \
