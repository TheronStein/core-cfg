--[[
  Manages globally shared toggleable terminal panes per window.
  Each terminal session is accessible from any tab/pane in the window.
  Can optionally be made truly global across all windows.
  Supports multiple independent terminal sessions with different IDs.
--]]
...
-- State for toggleable panes, keyed by [session_id][window_id]
-- Each session_id represents a different toggle terminal (e.g., "leader", "super")
-- Stores a single terminal pane per session per window that's shared across all tabs/panes
...
local function find_terminal_pane_in_window(window, terminal_pane_id)
...
--[[
  Core logic for toggling a specific terminal session.
  - Maintains one terminal pane per session per window (or globally if configured)
  - Creates the pane if it doesn't exist
  - Toggles between the terminal and the invoker pane
  - Moves the terminal to the current tab if it's in a different tab

  @param session_id string - Unique identifier for this terminal session (e.g., "leader", "super")
  @param opts table - Configuration options for this session (optional, uses defaults if nil)
]]
local function toggle_terminal_session(session_id, opts, window, pane)
	...
	if terminal_pane_exists then
		...
		if new_pane then
			...
			wezterm.log_info(
				"Created new terminal pane [" .. session_id .. "]. ID: " .. state.pane_id .. ", Invoker ID: " .. state.invoker_id
			)
...
--[[
  Create a toggle function for a specific terminal session.

  @param session_id string - Unique identifier for this terminal session
  @param opts table - Configuration options (optional)
  @return function - A function that can be used with wezterm.action_callback

  Example usage:
    local toggle_primary = toggle_terminal.create("primary", { direction = "Right", size = { Percent = 50 } })
    local toggle_scratch = toggle_terminal.create("scratch", { direction = "Bottom", size = { Percent = 30 } })
]]
function M.create(session_id, opts)
	return function(window, pane)
		toggle_terminal_session(session_id, opts, window, pane)
	end
end

-- Backward compatibility: default toggle_terminal function
function M.toggle_terminal(window, pane)
	toggle_terminal_session("default", default_opts, window, pane)
end
