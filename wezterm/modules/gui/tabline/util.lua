  ...
  for _, v in ipairs(components_opts) do
    if type(v) == 'string' then
      if v == 'ResetAttributes' then
        ...
        if DEBUG then
          wezterm.log_info('Attempting to load component: ' .. component_name)
        end
        local ok, result = pcall(require, component_name)
        if ok then
          if DEBUG then
            wezterm.log_info('Successfully loaded component: ' .. component_name)
          end
          ...
          if DEBUG then
            wezterm.log_error('Failed to load component ' .. component_name .. ': ' .. tostring(result))
          end
