	...
	window:perform_action(
		act.PromptInputLine({
			...
			action = wezterm.action_callback(function(win, p, workspace_name)
				...
				local existing_window_id = isolation.find_client_for_workspace(workspace_name)
				...
				tab_rename.show_icon_set_menu(win, p, function(inner_win, inner_pane, full_title, icon, title)
					...
					if current_workspace == "default" then
						...
						local success = isolation.spawn_workspace_client(workspace_name)
...
function M.switch_workspace(window, pane)
	...
	for _, ws in ipairs(all_workspaces) do
		...
		-- Check if workspace has running client
		local window_id = isolation.find_client_for_workspace(ws)
		local running_indicator = window_id and " 🟢" or ""
...
function M.close_workspace(window, pane)
	...
	for _, ws in ipairs(workspaces) do
		...
		-- Show if workspace has running client
		local window_id = isolation.find_client_for_workspace(ws)
		local running_indicator = window_id and " 🟢" or ""
	...
	window:perform_action(
		act.InputSelector({
			action = wezterm.action_callback(function(win, p, session_id)
				...
				-- Check if workspace already has a running client
...
function M.switch_workspace_to(window, pane, workspace_name)
	...
	local success = isolation.switch_to_workspace(workspace_name)

	if success then
		window:toast_notification("WezTerm", "📂 Switched to workspace: " .. workspace_name, nil, 2000)
	else
		window:toast_notification("WezTerm", "❌ Failed to switch workspace", nil, 2000)
	end
end
