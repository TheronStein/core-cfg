...
local function restore_session(window, pane, session_name)
  ...
  wezterm.log_info(
    "Restoring " .. #session.tabs .. " tabs in workspace " .. (session.workspace or session_name)
  )
  ...
  if first_tab_data.color then
    ...
    wezterm.log_info("Restored color for tab 1: " .. first_tab_data.color)
  end
  ...
  for i = 2, #session.tabs do
    ...
    if tab_data.color then
      ...
      wezterm.log_info("Restored color for tab " .. i .. ": " .. tab_data.color)
    end
  ...
  window:perform_action(
    act.InputSelector({
      action = wezterm.action_callback(function(win, p, id)
        ...
        local custom_tab_data = wezterm.GLOBAL.custom_tabs
          and wezterm.GLOBAL.custom_tabs[tab_id_str]
        local tab_icon = custom_tab_data and custom_tab_data.icon_key
        ...
        if mux_win then
          ...
          win:toast_notification("WezTerm", "✅ Grabbed tab from: " .. source_workspace, nil, 2000)
        else
          win:toast_notification("WezTerm", "❌ Failed to grab tab", nil, 2000)
        end
      end),
  ...
  window:perform_action(
    act.PromptInputLine({
      ...
      action = wezterm.action_callback(function(win, p, workspace_name)
        ...
        tab_rename.show_icon_set_menu(
          win,
          p,
          function(inner_win, inner_pane, full_title, icon, title)
            ...
            local display_name = (icon and icon ~= "") and (icon .. " " .. workspace_name)
              or workspace_name
            inner_win:toast_notification(
              "WezTerm",
              "✅ Created workspace: " .. display_name,
              nil,
              2000
            )
          end
        ...
        for _, tab in ipairs(mux_window:tabs()) do
          if tab:tab_id() == tab_id then
            ...
            if target_pane then
              ...
              win:toast_notification(
                "WezTerm",
                "✅ Moved pane to tab: " .. (tab:get_title() or "Untitled"),
                nil,
                2000
              )
              return
...
-- =====================================================
-- BASH-INTEGRATED PANE ACTIONS
-- =====================================================

-- Show bash pane management menu (actions submenu)
local function show_bash_pane_management_menu(window, pane)
  -- Create a simple menu with the pane management options
  local pane_choices = {
    { id = "back", label = "← Go Back to Main Menu" },
    { id = "separator", label = "─────────────────────────────" },
    { id = "pane_own_tab", label = "🚀 Move Pane to Own Tab" },
    { id = "pane_break", label = "📑 Break Pane to Tab (FZF)" },
    { id = "pane_grab", label = "📥 Grab Pane from Tab (FZF)" },
  }

  window:perform_action(
    act.InputSelector({
      action = wezterm.action_callback(function(win, p, pane_id)
        if pane_id == "back" then
          M.show_menu(win, p)
        elseif pane_id == "pane_own_tab" then
          move_pane_to_own_tab(win, p)
        elseif pane_id == "pane_break" then
          show_bash_break_pane_menu(win, p)
        elseif pane_id == "pane_grab" then
          show_bash_grab_pane_menu(win, p)
        end
      end),
...
-- Show bash FZF menu for breaking current pane to another tab
local function show_bash_break_pane_menu(window, pane)
  local mux_window = window:mux_window()
  local current_tab = window:active_tab()
  ...
  -- Build workspace data for the script
  local tabs = mux_window:tabs()
  local workspace_data = { tabs = {} }
  ...
  for _, tab in ipairs(tabs) do
    if tab:tab_id() ~= current_tab:tab_id() then
      ...
      -- Get custom tab icon
      local icon = ""
      if wezterm.GLOBAL.custom_tabs and wezterm.GLOBAL.custom_tabs[tab_id_str] then
        icon = wezterm.GLOBAL.custom_tabs[tab_id_str].icon_key or ""
      end

      -- Get first pane's cwd
      local cwd = ""
      if #tab_panes > 0 then
        cwd = extract_path(tab_panes[1]:get_current_working_dir())
      end

      table.insert(workspace_data.tabs, {
        id = tab:tab_id(),
        title = tab_title,
        icon = icon,
  ...
  local callback_file = wezterm.config_dir .. "/.data/break-pane-callback.tmp"

  -- Launch bash break-pane script with workspace data
  window:perform_action(
  ...
  local function watch_for_callback(iterations)
    ...
    if f then
      ...
      if action and action ~= "" then
        ...
        if action == "own_tab" then
          -- Break to own tab
          move_pane_to_own_tab(win, p)
        else
          -- Break to existing tab (action contains tab_id)
          local target_tab_id = tonumber(action)
          if target_tab_id then
            -- Find the target tab and move pane there
            for _, tab in ipairs(mux_window:tabs()) do
              if tab:tab_id() == target_tab_id then
                local cwd = extract_path(p:get_current_working_dir())
                local target_pane = tab:panes()[1]

                -- Create split in target tab
                local new_pane = target_pane:split({
                  direction = "Right",
                  cwd = cwd,
                })
                ...
                win:toast_notification("WezTerm", "✅ Pane moved to tab", nil, 2000)
                break
              end
            end
          end
        ...
        -- Close the break-pane script tab
          ...
          if mux_win then
            for _, tab in ipairs(mux_win:tabs()) do
              ...
              if #panes > 0 then
                ...
                if process and process:match("break%-pane%.sh") then
                  tab:activate()
...
-- Show bash FZF menu for grabbing a pane from another tab
local function show_bash_grab_pane_menu(window, pane)
  local mux_window = window:mux_window()
  local current_tab = window:active_tab()
  ...
  -- Build panes data for the script
  local panes_data = { panes = {} }
  ...
  if #panes_data.panes == 0 then
    window:toast_notification("WezTerm", "No panes available to grab", nil, 2000)
    return
  end

  -- Create callback file
  local callback_file = wezterm.config_dir .. "/.data/grab-pane-callback.tmp"

  -- Launch bash grab-pane script with panes data
  window:perform_action(
    wezterm.action.SpawnCommandInNewTab({
      args = {
  ...
  local function watch_for_callback(iterations)
    ...
    if f then
      ...
      if action and action ~= "" then
        ...
        if action == "no_panes" then
          win:toast_notification("WezTerm", "No panes available to grab", nil, 2000)
        else
          -- Action contains pane_id
          local target_pane_id = tonumber(action)
          if target_pane_id then
            -- Find the source pane and grab it
            for _, tab in ipairs(mux_window:tabs()) do
              for _, tab_pane in ipairs(tab:panes()) do
                if tab_pane:pane_id() == target_pane_id then
                  local cwd = extract_path(tab_pane:get_current_working_dir())

                  -- Create split in current tab
                  local new_pane = p:split({
                    direction = "Right",
                    cwd = cwd,
                  })
        ...
        -- Close the grab-pane script tab
        wezterm.time.call_after(0.2, function()
          local mux_win = win:mux_window()
          if mux_win then
            for _, tab in ipairs(mux_win:tabs()) do
              ...
              if #panes > 0 then
                local pane_obj = panes[1]
                local process = pane_obj:get_foreground_process_name()
                if process and process:match("grab%-pane%.sh") then
                  tab:activate()
                  wezterm.time.call_after(0.05, function()
                    win:perform_action(wezterm.action.CloseCurrentTab({ confirm = false }), pane_obj)
                  end)
...
function M.show_menu(window, pane)
  -- Create callback file for menu selection
  local callback_file = wezterm.config_dir .. "/.data/session-manager-callback.tmp"

  -- Launch the fzf session manager script
  window:perform_action(
    wezterm.action.SpawnCommandInNewTab({
      args = {
        paths.WEZTERM_SCRIPTS .. "/session-manager/session-manager.sh",
        callback_file,
      },
    }),
    pane
  ...
  -- Watch for menu selection result
  local function watch_for_selection(iterations)
    if iterations > 60 then
    ...
    local f = io.open(callback_file, "r")
    if f then
      local id = f:read("*line")
      f:close()
      os.remove(callback_file)

      if id and id ~= "" then
        -- Get window reference (might have changed)
        local win = window
        local p = pane

        -- Handle menu selection
        if id == "workspace_management" then
          local workspace_manager = require("modules.sessions.workspace_manager")
          workspace_manager.show_bash_menu(win, p)
              ...
              action = wezterm.action_callback(function(inner_win, inner_pane, theme_id)
                ...
                elseif theme_id == "theme_standard" then
                  ...
                  inner_win:perform_action(
                    wezterm.action.EmitEvent("start-theme-watcher"),
                    inner_pane
                  )
                ...
                elseif theme_id == "theme_popup" then
                  ...
                  local script_path = paths.WEZTERM_SCRIPTS
                    .. "/tmux-theme-browser/theme-browser-popup.sh"
          ...
          if mux_window then
            for _, tab in ipairs(mux_window:tabs()) do
              ...
              if #panes > 0 then
                local pane_obj = panes[1]
                local process = pane_obj:get_foreground_process_name()
                if process and process:match("session%-manager%.sh") then
                  tab:activate()
                  wezterm.time.call_after(0.05, function()
                    win:perform_action(wezterm.action.CloseCurrentTab({ confirm = false }), pane_obj)
                  end)
      ...
      -- File doesn't exist yet, check again
      wezterm.time.call_after(0.5, function()
        watch_for_selection(iterations + 1)
      end)
    end
  end

  -- Start watching for menu selection
  wezterm.time.call_after(0.5, function()
    watch_for_selection(0)
  end)
end
