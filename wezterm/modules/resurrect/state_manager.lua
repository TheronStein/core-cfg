...
---@param opt_name? string
function M.save_state(state, opt_name)
...
---@param type string
---@return table
function M.load_state(name, type)
...
function M.periodic_save(opts)
	...
	wezterm.time.call_after(opts.interval_seconds, function()
		...
		if opts.save_workspaces then
			M.save_state(require("resurrect.workspace_state").get_workspace_state())
		end
		...
		if opts.save_windows then
			for _, gui_win in ipairs(wezterm.gui.gui_windows()) do
				...
				if title ~= "" and title ~= nil then
					M.save_state(require("resurrect.window_state").get_window_state(mux_win))
				end
			end
		end
		...
		if opts.save_tabs then
			for _, gui_win in ipairs(wezterm.gui.gui_windows()) do
				...
				for _, mux_tab in ipairs(mux_win:tabs()) do
					...
					if title ~= "" and title ~= nil then
						M.save_state(require("resurrect.tab_state").get_tab_state(mux_tab))
					end
				end
			end
		end
...
---@return boolean
---@return string|nil
function M.write_current_state(name, type)
...
---@return boolean
---@return string|nil
function M.resurrect_on_gui_startup()
	local file_path = M.save_state_dir .. utils.separator .. "current_state"
	local suc, err = pcall(function()
		...
		if type == "workspace" then
			require("resurrect.workspace_state").restore_workspace(M.load_state(name, type), {
...
function M.delete_state(file_path)
	wezterm.emit("resurrect.state_manager.delete_state.start", file_path)
	local path = M.save_state_dir .. file_path
...
function M.set_max_nlines(max_nlines)
