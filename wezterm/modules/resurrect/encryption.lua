...
---@alias encryption_opts {enable: boolean, method: string, private_key: string?, Mlic_key: string?, encrypt: fun(file_path: string, lines: string), decrypt: fun(file_path: string): string}
...
---@param lines string
function M.encrypt(file_path, lines)
	local cmd = string.format("%s -r %s -o %s", M.method, M.Mlic_key, file_path:gsub(" ", "\\ "))

	if M.method:find("gpg") then
		cmd = string.format(
...
function M.decrypt(file_path)
	local cmd = { M.method, "-d", "-i", M.private_key, file_path }

	if M.method:find("gpg") then
		cmd = { M.method, "--batch", "--yes", "--decrypt", file_path }
	end
