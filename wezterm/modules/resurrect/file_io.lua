...
function M.set_encryption(user_opts)
	M.encryption = require("resurrect.encryption")
...
function M.write_state(file_path, state, event_type)
	...
	if M.encryption.enable then
		wezterm.emit("resurrect.file_io.encrypt.start", file_path)
		local ok, err = pcall(function()
			return M.encryption.encrypt(file_path, json_state)
		end)
		if not ok then
...
function M.load_json(file_path)
	...
	if M.encryption.enable then
		...
		local ok, output = pcall(function()
			return M.encryption.decrypt(file_path)
		end)
		if not ok then
