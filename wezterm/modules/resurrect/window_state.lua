...
function M.get_window_state(window)
	...
	-- Only get size if there are tabs
	if #tabs > 0 then
		window_state.size = tabs[1].tab:get_size()
	else
		-- Default size if no tabs
		window_state.size = { cols = 80, rows = 24 }
	end
				...
				wezterm.action.PromptInputLine({
					...
					action = wezterm.action_callback(function(window, _, title)
						if title then
							...
							local state = M.get_window_state(mux_win)
