...
function M.restore_workspace(workspace_state, opts)
	...
	for i, window_state in ipairs(workspace_state.window_states) do
		if i == 1 and opts.window then
			...
			-- Only set cwd if tabs exist
			if window_state.tabs and #window_state.tabs > 0 and window_state.tabs[1].pane_tree then
				spawn_window_args.cwd = window_state.tabs[1].pane_tree.cwd
			end
