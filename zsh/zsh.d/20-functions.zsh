# Start with only the most useful functions from the user's collection

# Basic utilities
function mkcd() { mkdir -p "$1" && cd "$1" }
# function bak() { cp -iv "$1" "$1.bak" }
...
function lsfuncs() {
    print -rl -- ${${(@o)${(k)functions[(I)^[_.+:@-]*]}}} | head -20
}

function zsh-stats() {
    echo "Functions: ${#${(k)functions}}"
    echo "Aliases: ${#${(k)aliases}}"
    echo "Plugins: $(zinit list | wc -l)"
    echo "Load time: $(zsh -ic 'exit' 2>&1 | grep real)"
}
...
function lswifi() { 
    if (( $+commands[nmcli] )); then
        nmcli device wifi
    elif (( $+commands[airport] )); then
        /System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport -s
    fi
}

# Network utilities (from user's wifi section)
function n1ip() { curl -s ipinfo.io/json | jq . 2>/dev/null || curl -s ipinfo.io; }

# Base conversion functions (useful subset from user's collection)
function h2d() { builtin print $(( [#10] 0x${1#0x} )); }
function d2h() { builtin print $(( [#16] $1 )); }

# FZF-powered functions (only if fzf exists)
...
# Git status for prompt (if not using powerline)
function git_info() {
    local ref
...
# Directory hash functions
if (( $+commands[fd] )); then
    function sha256dir() { 
        fd . -tf -x sha256sum 2>/dev/null | awk '{print $1}' | sort | sha256sum | awk '{print $1}'
    }
fi
...
# Advanced Zsh-specific functions from user's config

# List and manage Zsh functions
function lszle() {
...
function lscmds() {
    print -rl -- ${(ok)commands[@]} | ${PAGER:-less}
}

function lsfuncs() {
    emulate -L zsh -o extendedglob
    print -rl -- ${${(@o)${(k)functions[(I)^[_.+:@-]*]}}} | head -50
}

# Which completion is being used
function whichcomp() {
    for arg; do
        (( $+_comps[$arg] )) && {
            print "$arg uses $_comps[$arg]"
            print -l $^fpath/${_comps[$arg]}(N)
        }
    done
}

# Edit alias/function quickly
function ealias() {
    [[ -z "$1" ]] && { echo "Usage: ealias <alias>"; return 1; }
    vared aliases[$1]
}

function efunc() {
    [[ -z "$1" ]] && { echo "Usage: efunc <function>"; return 1; }
    autoload -Uz zed
    zed -f "$1"
}

# System management functions from user's config
...
function pid() {
    ps -ef | grep -v grep | grep "$@" | awk '{print $2}'
}
function mem() { ps aux | grep "^$USER" | awk '{sum+=$6} END {print sum/1024 " MB"}'; }
function psz() { ps -ef | grep -E "zsh|$$" | grep -v grep; }
# Memory and disk usage
function topmem() {
    ps aux | sort -nrk 6 | head -${1:-10}
}
...
# Clear history of sensitive commands
function history-clean() {
    ...
    for pattern in $patterns; do
        while history | grep -q "$pattern"; do
            history -d $(history | grep "$pattern" | head -1 | awk '{print $1}')
        done
    done
    echo "Cleaned sensitive entries from history"
}

# Advanced file management from user's config
...
# Recent files
function recent() {
    find . -type f -mtime -${1:-1} -ls | sort -k11
}

# Largest files
function largest() {
    find ${1:-.} -type f -exec ls -la {} \; 2>/dev/null | \
        sort -nrk5 | head -${2:-10}
}
...
function backup-ts() {
    for file in "$@"; do
        cp -v "$file" "${file}.$(date +%Y%m%d_%H%M%S).bak"
    done
}

# Backup functions (simplified versions)
function bak() {
...
function rbak() {
    [[ -z "$1" ]] && { echo "Usage: rbak <file>"; return 1; }
    [[ -f "$1.bak" ]] && mv -iv "$1.bak" "$1"
}
...
function bk() {
    ...
    cp -iv "$1" "${1:r}_$(date '+%Y_%m_%d')${${1:e}:+.${1:e}}"
}
...
function srm() {
    for file in "$@"; do
        if [[ -f "$file" ]]; then
            dd if=/dev/urandom of="$file" bs=1024 count=$(( $(stat -f%z "$file" 2>/dev/null || stat -c%s "$file") / 1024 + 1 )) 2>/dev/null
            rm -v "$file"
        fi
    done
}                         

# Language-specific utilities from user's config

# Rust utilities (if cargo is available)
if (( $+commands[cargo] )); then
    function cargo-home() {
        cargo show "${@}" --json 2>/dev/null | jq -r .crate.homepage
    }
    
    function cdc() {
        cd "$(cargo locate-project 2>/dev/null | jq -r .root | xargs dirname)"
    }
    
    # Edit cargo project files
    function cme() {
        local root=$(cargo locate-project 2>/dev/null | jq -r .root | xargs dirname)
        [[ -n "$root" ]] && ${EDITOR:-vim} "$root/src/main.rs"
    }
    
    function cml() {
        local root=$(cargo locate-project 2>/dev/null | jq -r .root | xargs dirname)
        [[ -n "$root" ]] && ${EDITOR:-vim} "$root/src/lib.rs"
    }
    
    function cmc() {
        local root=$(cargo locate-project 2>/dev/null | jq -r .root | xargs dirname)
        [[ -n "$root" ]] && ${EDITOR:-vim} "$root/Cargo.toml"
    }
fi

# Python utilities
if (( $+commands[python3] )); then
    # Simple Python REPL with math
    function py() {
        if [[ -n "$*" ]]; then
...
if (( $+commands[node] )); then
    function npm-list-global() {
        npm list -g --depth=0
    }
    
    function node-repl() {
        node -i -e "const fs = require('fs'); const path = require('path');"
    }
fi
...
function mybin_off() { path=( "${path[@]:#$CORE/bin}" ) }
# @desc: adds /$HOME/mybin to $PATH
function mybin_on()  { mybin_off; PATH=$PATH:$CORE/bin; }
