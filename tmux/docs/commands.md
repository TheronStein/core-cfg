## [COMMANDS](https://man.openbsd.org/tmux#COMMANDS)
...
4.  A [glob(7)](https://man.openbsd.org/glob.7) pattern which is matched against the session name.
...
6.  As a [glob(7)](https://man.openbsd.org/glob.7) pattern matched against the window name.
...
[**Token**](https://man.openbsd.org/tmux#Token)

[**Meaning**](https://man.openbsd.org/tmux#Meaning)

[`{start}`](https://man.openbsd.org/tmux#_start_)

^

The lowest-numbered window

[`{end}`](https://man.openbsd.org/tmux#_end_)

$

The highest-numbered window

[`{last}`](https://man.openbsd.org/tmux#_last_)

!

The last (previously current) window

[`{next}`](https://man.openbsd.org/tmux#_next_)

-

The next window by number

[`{previous}`](https://man.openbsd.org/tmux#_previous_)
...
[**Token**](https://man.openbsd.org/tmux#Token~2)

[**Meaning**](https://man.openbsd.org/tmux#Meaning~2)

[`{last}`](https://man.openbsd.org/tmux#_last_~2)

!

The last (previously active) pane

[`{next}`](https://man.openbsd.org/tmux#_next_~2)

-

The next pane by number

[`{previous}`](https://man.openbsd.org/tmux#_previous_~2)

\-

The previous pane by number

[`{top}`](https://man.openbsd.org/tmux#_top_)

The top pane

[`{bottom}`](https://man.openbsd.org/tmux#_bottom_)

The bottom pane

[`{left}`](https://man.openbsd.org/tmux#_left_)

The leftmost pane

[`{right}`](https://man.openbsd.org/tmux#_right_)

The rightmost pane

[`{top-left}`](https://man.openbsd.org/tmux#_top-left_)

The top-left pane

[`{top-right}`](https://man.openbsd.org/tmux#_top-right_)

The top-right pane

[`{bottom-left}`](https://man.openbsd.org/tmux#_bottom-left_)

The bottom-left pane

[`{bottom-right}`](https://man.openbsd.org/tmux#_bottom-right_)

The bottom-right pane

[`{up-of}`](https://man.openbsd.org/tmux#_up-of_)

The pane above the active pane

[`{down-of}`](https://man.openbsd.org/tmux#_down-of_)

The pane below the active pane

[`{left-of}`](https://man.openbsd.org/tmux#_left-of_)

The pane to the left of the active pane

[`{right-of}`](https://man.openbsd.org/tmux#_right-of_)
...
In addition, [_target-session_](https://man.openbsd.org/tmux#target-session), [_target-window_](https://man.openbsd.org/tmux#target-window) or [_target-pane_](https://man.openbsd.org/tmux#target-pane) may consist entirely of the token ‘`{mouse}`’ (alternative form ‘`=`’) to specify the session, window or pane where the most recent mouse event occurred (see the [MOUSE SUPPORT](https://man.openbsd.org/tmux#MOUSE_SUPPORT) section) or ‘`{marked}`’ (alternative form ‘`~`’) to specify the marked pane (see `select-pane` `-m`).

Sessions, window and panes are each numbered with a unique ID; session IDs are prefixed with a ‘`$`’, windows with a ‘`@`’, and panes with a ‘`%`’. These are unique and are unchanged for the life of the session, window or pane in the `tmux` server. The pane ID is passed to the child process of the pane in the `TMUX_PANE` environment variable. IDs may be displayed using the ‘`session_id`’, ‘`window_id`’, or ‘`pane_id`’ formats (see the [FORMATS](https://man.openbsd.org/tmux#FORMATS) section) and the `display-message`, `list-sessions`, `list-windows` or `list-panes` commands.

shell-command arguments are [sh(1)](https://man.openbsd.org/sh.1) commands. This may be a single argument passed to the shell, for example:
...
Will run [vi(1)](https://man.openbsd.org/vi.1) directly without invoking the shell.
...
Or from [sh(1)](https://man.openbsd.org/sh.1):
