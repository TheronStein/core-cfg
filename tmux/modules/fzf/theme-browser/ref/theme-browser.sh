#!/bin/bash
# ~/.core/cfg/wezterm/scripts/theme-browser.sh
# Enhanced WezTerm Theme Browser with live preview and session persistence

set -e

# Configuration paths
WEZTERM_DATA="$HOME/.core/cfg/wezterm/data"
WEZTERM_SESSIONS="$HOME/.core/cfg/wezterm/sessions"
...
init() {
    ...
    if [[ ! -f "$THEMES_JSON" ]]; then
        ...
        if command -v wezterm >/dev/null 2>&1; then
            wezterm --config-file "$HOME/.core/cfg/wezterm/scripts/init-themes.lua" >/dev/null 2>&1 || {
                # Fallback: run as lua script
                lua "$HOME/.core/cfg/wezterm/scripts/init-themes.lua" 2>/dev/null || {
    ...
    if [[ ! -f "$EXPORT_JSON" ]] && [[ ! -f "$THEMES_JSON" ]]; then
        ...
        echo -e "${YELLOW}Please run: lua ~/.core/cfg/wezterm/scripts/init-themes.lua${NC}" >&2
        exit 1
    fi
...
apply_to_session() {
    ...
    if [[ -n "$session" ]]; then
        ...
        local session_file="$HOME/.core/cfg/sessions/tmux/${session}.lua"
