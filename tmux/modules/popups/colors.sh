...
# Popup Window Handler for Colors
# Opens neovim with all color theme files
# Auto-closes after 10 minutes of inactivity

SESSION="apps"
WINDOW_NAME="colors"
THEME_DIR="$HOME/.core/cfg/themes"
...
# Check if colors window already exists
...
if [ -z "$t_window" ]; then
  ...
  tmux send-keys -t "$SESSION:$next_window" "nvim \
    ...
    '${THEME_DIR}/colors.zsh' \
    '${THEME_DIR}/tabline-theme.lua' \
    '${THEME_DIR}/theme.css'" C-m
...
if [ "$CURRENT_SESSION" = "$SESSION" ]; then
  ...
  tmux display-popup -E -w 90% -h 90% -x C -y C \
    -T " Colors " -b rounded -S "fg=#89b4fa,bg=#1e1e2e" \
