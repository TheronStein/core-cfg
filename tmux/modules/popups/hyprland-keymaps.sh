...
# Popup Window Handler for Hyprland Keymaps
# Opens neovim with all Hyprland keymap files
# Auto-closes after 10 minutes of inactivity

SESSION="apps"
WINDOW_NAME="hypr-keys"
KEYMAP_DIR="$HOME/.core/env/desktop/hypr/keymaps"
...
# Check if hyprland keymaps window already exists
...
if [ -z "$t_window" ]; then
  ...
  tmux send-keys -t "$SESSION:$next_window" "nvim \
    ...
    -c 'set updatetime=10000' \
    '${KEYMAP_DIR}/apps.conf' \
    '${KEYMAP_DIR}/core.conf' \
    '${KEYMAP_DIR}/devices.conf' \
    '${KEYMAP_DIR}/keyboard-layouts.conf' \
    '${KEYMAP_DIR}/volume.conf' \
    '${KEYMAP_DIR}/workspaces.conf' \
    '${KEYMAP_DIR}/keytables/windows.conf'" C-m
...
if [ "$CURRENT_SESSION" = "$SESSION" ]; then
  ...
  tmux display-popup -E -w 90% -h 90% -x C -y C \
    -T " Hyprland Keymaps " -b rounded -S "fg=#89b4fa,bg=#1e1e2e" \
