#!/bin/bash
# Bin Scripts Menu
# Location: ~/.tmux/modules/menus/config/bin-config-menu.sh

source "$TMUX_MENUS/menu-settings.sh"

CURRENT_MENU="config/bin-config-menu.sh"
PARENT=$(get_parent "config-management.sh")

TOOL_NAME="bin"
CFG_DIR="$HOME/.local/bin"

tmux display-menu -x $MENU_POS_X -y $MENU_POS_Y \
  -T "$(menu_title '󰏗' 'Bin Scripts' $MENU_TITLE_CONFIG)" \
