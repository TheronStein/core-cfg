#!/bin/bash
# GitHub CLI Configuration Menu
# Location: ~/.tmux/modules/menus/config/gh-config-menu.sh

source "$TMUX_MENUS/menu-settings.sh"

CURRENT_MENU="config/gh-config-menu.sh"
PARENT=$(get_parent "config-management.sh")

TOOL_NAME="gh"
CFG_DIR="$HOME/.config/gh"

tmux display-menu -x $MENU_POS_X -y $MENU_POS_Y \
  -T "$(menu_title '' 'GitHub CLI Configuration' $MENU_TITLE_CONFIG)" \
