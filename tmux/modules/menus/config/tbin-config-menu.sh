#!/bin/bash
# TBin Scripts Menu
# Location: ~/.tmux/modules/menus/config/tbin-config-menu.sh

source "$TMUX_MENUS/menu-settings.sh"

CURRENT_MENU="config/tbin-config-menu.sh"
PARENT=$(get_parent "config-management.sh")

TOOL_NAME="tbin"
CFG_DIR="$HOME/.local/tbin"

tmux display-menu -x $MENU_POS_X -y $MENU_POS_Y \
  -T "$(menu_title '󰏗' 'TBin Scripts' $MENU_TITLE_CONFIG)" \
  ...
  "" \
  "$(menu_sep 'Edit')" "" "" \
  " Edit Scripts" e "run-shell 'source $TMUX_CONF/lib/config-session.sh && edit_config $TOOL_NAME \"$CFG_DIR\"'" \
