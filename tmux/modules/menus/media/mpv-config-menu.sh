#!/bin/bash
# MPV Configuration Menu
# Location: ~/.tmux/modules/menus/media/mpv-config-menu.sh

source "$TMUX_MENUS/menu-settings.sh"

CURRENT_MENU="media/mpv-config-menu.sh"
PARENT=$(get_parent "app-management.sh")

TOOL_NAME="mpv"
CFG_DIR="$HOME/.config/mpv"

tmux display-menu -x $MENU_POS_X -y $MENU_POS_Y \
  -T "$(menu_title '󰐹' 'MPV Configuration' $MENU_TITLE_APP)" \
