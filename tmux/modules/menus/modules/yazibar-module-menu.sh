...
# Location: ~/.tmux/modules/menus/modules/yazibar-module-menu.sh

source "$TMUX_MENUS/menu-settings.sh"

CURRENT_MENU="modules/yazibar-module-menu.sh"
PARENT=$(get_parent "tmux/modules-menu.sh")

TOOL_NAME="yazibar"
CFG_DIR="$HOME/.core/.sys/cfg/tmux/modules/yazibar"

tmux display-menu -x $MENU_POS_X -y $MENU_POS_Y \
  -T "$(menu_title '' 'Yazibar Configuration' $MENU_TITLE_MODULE)" \
